    ...
    return (
      <div className="min-h-screen">
        ...
        <div className="mx-auto max-w-md px-4 py-20 text-center">
          ...
          <Link
            to="/login"
            className="mt-4 inline-flex h-11 items-center rounded-full bg-gradient-warm px-6 font-semibold text-primary-foreground shadow-warm"
          >
            Se connecter
          </Link>
        </div>
        ...
        <div className="mx-auto max-w-md px-4 py-20 text-center">
          ...
          <p className="mt-3 text-muted-foreground">
            ...
            <span className="font-mono font-semibold text-foreground">
              #{confirmed.id.slice(-8)}
            </span>{" "}
            a été envoyée au cuisinier.
          </p>
          <p className="mt-1 text-2xl font-display font-bold text-primary">
            {confirmed.total.toFixed(2)} DT
          </p>
          <p className="mt-4 text-sm text-muted-foreground">
            Le cuisinier va l'accepter ou la refuser. Vous serez contacté par téléphone.
          </p>
          <Link
            to="/plats"
            className="mt-8 inline-flex h-12 items-center rounded-full bg-gradient-warm px-7 font-semibold text-primary-foreground shadow-warm"
          >
          ...
          <div className="space-y-6">
            ...
            <section className="rounded-2xl border border-border bg-card p-6 shadow-soft">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-3">
                ...
                >
                  <Banknote
                    className={`h-6 w-6 ${paiement === "livraison" ? "text-primary" : "text-muted-foreground"}`}
                  />
                  <div>
                ...
                >
                  <Smartphone
                    className={`h-6 w-6 ${paiement === "d17" ? "text-primary" : "text-muted-foreground"}`}
                  />
                  <div>
                ...
                >
                  <CreditCard
                    className={`h-6 w-6 ${paiement === "carte" ? "text-primary" : "text-muted-foreground"}`}
                  />
                  <div>
              ...
              {paiement === "d17" && (
                <div className="mt-6 space-y-4 rounded-xl border border-border bg-background/60 p-4">
                  ...
                  <p className="text-xs text-muted-foreground">
                    💡 Vous recevrez une notification sur votre application D17 pour valider le
                    paiement.
                  </p>
                </div>
              )}
              ...
              {paiement === "livraison" && (
                <div className="mt-6 rounded-xl border border-dashed border-border bg-background/60 p-4 text-sm text-muted-foreground">
                  Vous payerez en espèces directement au cuisinier ou au livreur lors de la
                  réception.
                </div>
              )}
            </section>
