    ...
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-3">
          ...
          <StatCard
            label="En attente"
            value={myOrders.filter((o) => o.status === "en_attente").length}
            icon={AlertCircle}
          />
        </div>
            ...
            {myDishes.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-border bg-card/50 p-12 text-center">
                ...
                <button
                  ...
                  className="mt-4 inline-flex h-11 items-center gap-2 rounded-full bg-gradient-warm px-5 font-semibold text-primary-foreground shadow-warm"
                >
            ...
            {myOrders.length === 0 ? (
              ...
              myOrders.map((o) => {
                const myItems = o.items.filter(
                  (i) => i.cuisinier.toLowerCase() === user.nom.toLowerCase(),
                );
...
function AjouterPlatForm({
  ...
  return (
    <form
      ...
      className="mt-8 grid gap-4 rounded-2xl border border-border bg-card p-6 shadow-soft sm:grid-cols-2"
    >
      <div className="sm:col-span-2">
        <h2 className="font-display text-xl font-semibold">Nouveau plat</h2>
        <p className="text-sm text-muted-foreground">
          Le plat sera publié après validation par un administrateur.
        </p>
      </div>
      ...
      <Field label="Ingrédients (séparés par virgule)" full>
        <input
          ...
          className={inputCls}
          placeholder="Semoule, Agneau, Carottes..."
        />
      </Field>
      ...
      <Field label="Votre bio (courte)" full>
        <input
          ...
          className={inputCls}
          placeholder="Cuisinier passionné depuis 10 ans..."
        />
      </Field>
      ...
      {success && (
        <p className="rounded-lg bg-emerald-500/10 px-3 py-2 text-sm text-emerald-700 dark:text-emerald-400 sm:col-span-2">
          ✓ Plat soumis ! En attente de validation par un admin.
        </p>
      )}
      ...
      <div className="sm:col-span-2">
        <button
          type="submit"
          className="h-12 w-full rounded-full bg-gradient-warm font-semibold text-primary-foreground shadow-warm transition-smooth hover:opacity-90 sm:w-auto sm:px-8"
        >
