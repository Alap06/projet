...
function DashboardAdmin() {
  ...
  const validOrders = orders.filter((o) =>
    ["acceptee", "en_livraison", "livree"].includes(o.status),
  );
    ...
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-4">
          <StatCard
            label="En attente"
            value={pendingDishes.length}
            icon={Clock}
            highlight={pendingDishes.length > 0}
          />
          <StatCard
            label="Plats publiés"
            value={dishes.filter((d) => (d.status ?? "approved") === "approved").length}
            icon={Package}
          />
          <StatCard label="Commandes (validées)" value={validOrders.length} icon={ShoppingBag} />
          <StatCard
            label="Commission (10%)"
            value={`${commission.toFixed(2)} DT`}
            icon={DollarSign}
            highlight
          />
        </div>
          ...
          {[
            {
              ...
              label: `Validation ${pendingDishes.length > 0 ? `(${pendingDishes.length})` : ""}`,
            },
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            par <span className="font-semibold">{d.cuisinier}</span> • {d.ville}
                          </p>
                        </div>
...
function StatCard({
  ...
  return (
    <div
      className={`rounded-2xl border p-5 shadow-soft ${highlight ? "border-primary bg-primary/5" : "border-border bg-card"}`}
    >
